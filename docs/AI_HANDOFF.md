...
## Current Status (2026-05-03 — PR 4.PH)

**Phase 4 Roguelike Progression — PR 4.PA through PR 4.PH code landed.** PR 4.PH adds a run-scoped `RunStatsTracker` singleton, a Run Stats panel on Victory/GameOver, an extended `ArenaRuntimeDebugOverlay` (TZ §19 fields), and gated `debugLog` flags on `RunProgressionController` / `ShopController` / `RestRoomController` for tuning. Aggressive numeric balance tuning was intentionally deferred — TZ acceptance ("5–7 upgrades before boss, shop forces choices, Elite risky-but-rewarding") needs real Editor playtest data; the new logs + stats screen are the tuning surface for that pass. `dotnet build Assembly-CSharp.csproj --no-restore` passes with 0 errors. Detailed scope in [PROGRESS.md](./PROGRESS.md) Phase 4 section + changelog block 2026-05-03.

**Suggested next direction:** Editor playtest pass over PR 4.PE/PF/PG/PH — full 10-room run with `debugLog` enabled, validate stats screen + debug overlay, then iterate on numeric balance based on observed KP/upgrade pacing. After that, PR 4.PI Scenario Playtest Pass (S1-S10 from TZ).
...
- **PR 4.PH — Balance Pass + Run Stats.** New `Assets/Scripts/Progression/RunStatsTracker.cs` (namespace `VoidSurvivor.Progression`) — auto-singleton run-scoped aggregator. Subscribes to `Health.AnyDeath` / `Health.AnyDamaged` / `UpgradeSystem.OnUpgradesChanged` / `UpgradeSystem.OnGloryKill` / `KillPointsWallet.OnTotalChanged`. Tracks kills (excluding player + non-enemies), brute kills (Tank role), glory kills, damage taken, deaths, KP earned/spent, arena/shop/rest/elite visits via `NotifyArenaCleared(category)` (called from `RunController.OnArenaBuilt`'s Cleared lambda), full upgrade list re-derived from `UpgradeSystem.ActiveUpgrades`, and run duration. `RunStatsSnapshot` struct exposes the totals. `RunController.StartRun` calls `ResetForNewRun()`; `ShowScreen` calls `StopRun()` then `FormatStats(snapshot)` to populate the new `statsText` Text below the title. `ArenaRuntimeDebugOverlay` extended (TZ §19) — `showDetailed` bool (default true) appends KP / raw style / reward tier row / enemy budget + HP multiplier / active upgrades comma-list / pending rare boost flag; canvas resized to 560×260. Three new independent `debugLog` Inspector flags: `RunProgressionController.debugLog` (reward picks + rarity, payout breakdown), `ShopController.debugLog` (reroll inventory + per-purchase wallet delta), `RestRoomController.debugLog` (selection). Numeric balance left untouched until playtest data exists.

- **PR 4.PG — Rest Room + Final Prep.** `RestOption` + `RestRoomController` (auto-singleton, mirror of `ShopController`) + `RestRoomCanvas` (runtime UI, sortingOrder 5900) + `RestTerminalTrigger`. Three options: (1) Heal 35% max HP, (2) +10 max HP for the rest of the run, (3) Spend 15 KP to boost rarity of the next reward draw. Rest barriers stay closed until a choice is made — `RestRoomController.PrepareForArena` sets `encounter.HoldBarriers=true` and the `EncounterController.Start` patch keeps barriers closed for `ClearCondition.None` when `HoldBarriers` is preset. Esc closes UI but keeps gate locked; stepping off and back onto the platform reopens. Heal calls `Health.Heal`, +Max HP bumps `Health.maxHealth` (one-shot Rest reward, not stacking), Prime spends KP via `KillPointsWallet.TrySpend` and calls new `RunProgressionController.QueueRareRewardBoost()`. `RewardCardGenerator.Generate` consumes the boost on next call (additive: -20% Common / +12% Rare / +6% Epic / +2% Legendary, normalized, stacks on Elite). `RunProgressionController.ResetForNewRun()` wipes `pendingRareBoost` + `rewardCounter`; `RunController.StartRun` calls it. `ArenaBuilder.BuildSingleRestTerminal` spawns the green platform (base + soft glow insert + cross + benches + console + light + slow upward particles + new `RestPlatformGlow(Runtime)` unlit/emissive material). Final Prep node already wired in PR 4.PD's `StageTemplates[8]`.
...
**Editor playtest status:** user confirmed PR 4.PE + PR 4.PF Shop platform interaction worked. PR 4.PG just landed — focused Unity Editor playtest is the next concrete step (gating + 3 options + Prime → next draw effect; restart wipes `pendingRareBoost`).
