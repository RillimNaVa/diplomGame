...
| Date | What was done |
|------|---------------|
| 2026-04-22 | Phase 2 PR 2.C (NavMesh + Encounter Integration) — code merged (commit b1ae99f), Editor verification pending. New `Assets/Scripts/ProceduralArena/Navigation/ArenaNavMeshController.cs` — attaches `NavMeshSurface` to ArenaRoot и делает async `UpdateNavMesh` (использует `com.unity.ai.navigation` 2.0.9, уже в manifest; lifecycle auto-cleans on ArenaRoot destroy). New `Assets/Scripts/ProceduralArena/Encounter/` — `SoftLockBarrier` (Open/Close toggles collider+renderer на emissive orange cube), `EncounterTrigger` (one-shot OnTriggerEnter volume — scaffold, current flow его обходит, т.к. player телепортируется *внутрь* арены), `EncounterController` (per-arena MonoBehaviour на ArenaRoot — holds spawnPoints + barriers + clearCondition; `BeginEncounter` routes в `GameManager.BeginEncounter` для KillAll, auto-clears для None, ReachExit ждёт `FinishByReach()`). `GameManager.cs` — new `useEncounterMode` flag (gates legacy wave loop), `SetSpawnPoints` / `BeginEncounter(count, spawns, onKilledCallback)` / `EndEncounter()` API; encounter spawn coroutine reuses enemyPrefab + SimpleEnemyAI. `SimpleEnemyAI.Update` wraps `SetDestination` с `agent.isOnNavMesh` guard (prevents exceptions during first frames of runtime bake). `ArenaBuildMaterials.barrier` — orange emissive URP Lit. `ArenaFlowController.EnterArena` flow: fade → destroy+build → `SpawnExitTriggers` (trigger + fall-barrier + SoftLockBarrier cube + label per exit) → `ArenaNavMeshController.BakeAsync` → `SetupEncounter` (creates CombatSpawns под ArenaRoot из `room.combatSpawnPoints` + arenaParent offset, attaches EncounterController) → teleport → fadeOut → `encounter.BeginEncounter()`. `RunController.OnArenaBuilt` subscribes to `EncounterController.Cleared` → `NotifyArenaClearedIfReady`; при `skipClearCondition=true` переопределяет `clearCondition=None` для walk-through debugging. |
